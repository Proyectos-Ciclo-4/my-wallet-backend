...
public class Validators {
  ...
  public Mono<AnadirContacto> validateUserExists(Mono<AnadirContacto> command) {
    return command.flatMap(anadirContacto -> {
      log.info("Validating user {} exists", anadirContacto);

      return usuarioRepositorio.obtenerDatosUsuario(anadirContacto.getEmail(),
          anadirContacto.getTelefono()).flatMap(usuarioData -> {

        if (!usuarioData) {
          log.error("User doesn't have an account associated");
          return Mono.error(
              new RuntimeException("El usuario no tiene una cuenta o wallet asociada"));
        }
